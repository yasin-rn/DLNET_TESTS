...
int main()
{
	...
	Tensor<float> EncoderOutput2D({ BatchSize * BeamSize * SeqLen,ModelSize });
	...
	Functions::FillWithCoeff(EncoderOutput2D, 6.0f, 5.0f, 4.0f);
	std::cout << "Input2D :\n" << Input2D.ToString() << "\n\n";
	std::cout << "EncoderOutput2D :\n" << EncoderOutput2D.ToString() << "\n\n";

	Tensor<float> Input = Input2D.Reshape({ BatchSize,BeamSize,SeqLen,ModelSize });
	Tensor<float> EncoderOutput = EncoderOutput2D.Reshape({ BatchSize,BeamSize,SeqLen,ModelSize });
	...
	TransformerDecoderLayer<float> DecoderLayer(ModelSize, SeqLen, HiddenSize, NumOfHead, DLNET_ACTIVATION_RELU);

	DecoderLayer.SetMaskedMhaWeights(SelfAttn_Weights);
	DecoderLayer.SetMhaWeights(MultiheadAttn_Weights);
	DecoderLayer.SetLinear1Weights(Linear1_W);
	DecoderLayer.SetLinear2Weights(Linear2_W);

	auto Decoder_Out = DecoderLayer.Forward(Input, EncoderOutput);

	std::cout << "Decoder_Out :\n" << Decoder_Out.ToString() << "\n\n";
